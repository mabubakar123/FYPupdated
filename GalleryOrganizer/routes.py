...
def videoclip():
    ...
    if request.method=="POST" and 'array' in request.form:
        ...
        if trans=="left":
            ...
            final_clip.write_videofile(video_path+'my_video.mp4', fps=total_fps)
        elif trans=="right":
            slided_clips = [CompositeVideoClip([
       ...
       # clip = ImageSequenceClip(image_files, fps=fps)
        ...
        my_clip = mpe.VideoFileClip(video_path+'my_video.mp4')
        ...
        final.write_videofile(path_to_download_folder+"/VideoClip.mp4")
        final.close()
        final_clip.close()
        return jsonify({'success' : 'Video is generated and downloaded on your machine!'})
    form = SortForm()
    chosen = form.sort_by.data
    if chosen == 1:
        images = G_Image.query.filter_by(user_id=current_user.id).order_by(G_Image.image_actual_name).all()
        ...
        images = G_Image.query.filter_by(user_id=current_user.id).order_by(G_Image.image_file_size.desc()).all()
    return render_template('videoclip.html', images = images, form=form)
    images = G_Image.query.filter_by(user_id=current_user.id)
    return render_template('videoclip.html', title='Images', images = images, form=form)
